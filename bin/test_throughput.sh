...
./$PROGRAM -t 5 -h 5 $HOST:$PORT/file_small > $WORKSPACE/test
...
./$PROGRAM -t 5 -h 5 $HOST:$PORT/file_medium > $WORKSPACE/test
if ! check_status $? 0; then
    error "Failure"
else
    grep_latency $WORKSPACE/test
...
./$PROGRAM -t 2 -h 2 $HOST:$PORT/file_large > $WORKSPACE/test
if ! check_status $? 0; then
    error "Failure"
else
    grep_latency $WORKSPACE/test
