...
./$PROGRAM -t 10 -h 2 $HOST:$PORT/small.txt > $WORKSPACE/test
...
./$PROGRAM -t 10 -h 2 $HOST:$PORT/medium.txt > $WORKSPACE/test
if ! check_status $? 0; then
    error "Failure"
else
    grep_latency $WORKSPACE/test
...
./$PROGRAM -t 10 -h 2 $HOST:$PORT/large.txt > $WORKSPACE/test
if ! check_status $? 0; then
    error "Failure"
else
    grep_latency $WORKSPACE/test
